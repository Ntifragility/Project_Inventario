-- 1. Ensure metrado_ot exists (from the previous script)
ALTER TABLE consumos_campo 
ADD COLUMN IF NOT EXISTS metrado_ot NUMERIC(10, 2) DEFAULT 0;

-- 2. Ensure cant_oc exists (from this script)
ALTER TABLE movimientos 
ADD COLUMN IF NOT EXISTS cant_oc NUMERIC(10, 2) DEFAULT 0;

-- 3. DROP the old view first to avoid the PostgreSQL "cannot change name of view column" error
DROP VIEW IF EXISTS v_balance_consumos;

-- 4. Recreate the view with the new columns included
