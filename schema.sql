...
-- 1e. Product Synonyms / Dictionary Table (replaces external Tabla_Equiv)
CREATE TABLE IF NOT EXISTS productos_sinonimos (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    producto_codigo VARCHAR(50) NOT NULL REFERENCES productos(codigo) ON DELETE CASCADE,
    texto_sinonimo TEXT NOT NULL,
    tipo_columna VARCHAR(20) DEFAULT 'DESCRIPCION' CHECK (tipo_columna IN ('DESCRIPCION', 'TXT_LARGO', 'TXT_POS')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL,
