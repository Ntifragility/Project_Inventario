...
export default function SmartImportWizard({ user, onClose, onImportComplete }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < matchedRows.length; i++) {
      ...
      preview.push(buildPreviewRow(row, row._matchedCodigo, 'auto', i));
    }
    ...
    for (let i = 0; i < unmatchedRows.length; i++) {
      ...
      const desc = row._description;
      if (resolutions[desc]) {
        preview.push(buildPreviewRow(row, resolutions[desc], 'resolved', matchedRows.length + i));
      }
    }
  ...
  const buildPreviewRow = (row, productCodigo, matchType, idx = 0) => {
    ...
    // Transaction key (auto-generated if missing or unmapped)
    let transactionKey = String(row.key || '').trim();
    let txKeyAutoGenerated = false;
    if (!transactionKey) {
      const todayStr = new Date().toISOString().slice(0, 10).replace(/-/g, '');
      transactionKey = `AUTO-${config?.movementType || 'MOV'}-${todayStr}-${idx + 1}`;
      txKeyAutoGenerated = true;
    }
    ...
    return {
      ...
      _txKeyAutoGenerated: txKeyAutoGenerated
    };
  };
