...
import { Save, Upload, AlertCircle, CheckCircle2, Info, X, Eye, Clock, Scan, Pencil, Trash2, ChevronDown, ChevronUp, Download, Zap, FileSpreadsheet } from 'lucide-react';
...
export default function Movements({ user }) {
  ...
  const handleUndoLastImport = async () => {
    ...
    try {
      ...
      if (lastImport.type === 'ingreso') {
        setCsvMsg(successMsg);
      } else {
        setExcelMsg(successMsg);
      }
      setLastImport(null);
      fetchLedgerMovements();
    } catch (err) {
      console.error('Undo error:', err);
      const errorMsg = { text: 'Error al deshacer importación: ' + err.message, type: 'error' };
      if (lastImport.type === 'ingreso') {
        setCsvMsg(errorMsg);
      } else {
        setExcelMsg(errorMsg);
      }
    } finally {
      setUndoing(false);
    }
    ...
    <div id="movimientos" className="tab-content active">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="actions">
              ...
              >
                ...
                <span>Importar Movimientos (Excel/CSV)</span>
              </button>
              <button 
                type="button" 
                className="btn btn-secondary" 
      ...
      {showSmartWizard && (
        <SmartImportWizard
          ...
          onImportComplete={() => {
            fetchLedgerMovements();
            setFormMsg({ text: 'Importación exitosa.', type: 'success' });
          }}
        />
      )}
    </div>
